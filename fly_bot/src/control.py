...
def PID_pos(x,y,u,v):
	global x_setpoint, y_setpoint, roll_setpoint, pitch_setpoint, u_set, v_set
	...
	if(deltaT <= 4):
		...
		y_setpoint = 0
		point_a = True
	elif((u**2 + v**2)**(0.5) <= 0.1 and (err_x**2 + err_y**2)**(0.5) <= 0.2 and point_a == True):
		x_setpoint = 9
		y_setpoint = 0
		point_a = False
		point_b = True
	elif((u**2 + v**2)**(0.5) <= 0.1 and (err_x**2 + err_y**2)**(0.5) <= 0.2 and point_b == True):
		x_setpoint = 0
		y_setpoint = 9
		point_b = False
		point_c = True
	elif((u**2 + v**2)**(0.5) <= 0.1 and (err_x**2 + err_y**2)**(0.5) <= 0.2 and point_c == True):
		x_setpoint = 0
		y_setpoint = 0
		point_c = False
	...
	if((err_x**2 + err_y**2)**(0.5) <= 1.5):
		...
		pitch_setpoint = -output_x
		roll_setpoint = output_y
		...
		pitch_setpoint = -output_u
		roll_setpoint = output_v
...
def PID(roll, pitch, yaw, altitude, f):
	...
	alt_setpoint = 1.5
	yaw_setpoint = 0
	err_pitch = float(pitch)*(180 / 3.141592653) - pitch_setpoint 
 	err_roll = float(roll)*(180 / 3.141592653) - roll_setpoint
	err_yaw = float(yaw)*(180/3.14159263) - yaw_setpoint
	...
	#print('kp '+ str(pMem_roll))
	#print('ki '+str(iMem_roll*ki_roll))
	#print('kd '+ str(dMem_roll*kd_roll))
